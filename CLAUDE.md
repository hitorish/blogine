...
데모 흐름: 사용자가 웹 UI에서 이미지+텍스트 업로드 → 백엔드가 헤드리스로 호출 → 결과 HTML을 iframe으로 재생.

## Headless behavior rules (READ FIRST)

당신은 라이브 데모의 백엔드 워커로 호출됩니다. 사용자는 키보드 앞에 없습니다.

- **질문 금지** — clarifying question 절대 하지 말 것
- **옵션 나열 금지** — "Which one should I…" 류의 출력 금지
- **설명 금지** — "Here's what I'll do…" 도입부 금지
- **작업하고 종료** — 곧바로 도구 호출로 들어가서 결과만 만들고 끝낼 것
- **세션 격리** — 프롬프트가 지정한 세션 ID만 처리. `sessions/` 아래 다른 디렉터리 읽지/언급하지 말 것
- **빈 `content.txt`는 의도된 입력** — "비어있어서 못 만든다"가 아니라, 이미지에서 주제를 추론해 직접 카피라이팅 작성. 이 동작이 정상이고 가장 흔한 케이스
- **출력 경로 고정** — 결과는 항상 `sessions/<id>/output/`에 작성, 마지막에 `sessions/<id>/output/.done`에 `ok` 작성
